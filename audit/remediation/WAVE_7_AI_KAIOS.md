...
| AI_SCORE_AFTER (implementation) | 93/100 | live provider canary not run |
| KAIOS_ARCHITECTURE_SCORE | 93/100 | specs + compiler + orchestrator + combined vision envelope + relevant-only memory |
| AI_SAFETY_SCORE | 93/100 | history/memory/tools/JSON/combined-envelope fail-closed tested; live injection unproven |
| AI_RELIABILITY_SCORE | 90/100 | abort/quota/idempotency/same-image reuse unit-tested; no live soak |
| AI_COST_EFFICIENCY_SCORE | 92/100 | photo 3→2 calls; Leo repeat Gemini 0 when reuse valid; no periodic summary LLM |
| COST_EFFICIENCY_PRODUCT_SCORE_BEFORE | 86/100 | Wave 5 |
| COST_EFFICIENCY_PRODUCT_SCORE_AFTER | 92/100 | relative provider-call reduction only; no dollar claim |

95+ on AI was **not** awarded: live DeepSeek/Gemini canary is deferred (Wave 8 / MANUAL_CANARY). Implementation residuals from the first Wave 7 pass (separate quality Gemini, unwired Leo reuse, fill-to-five memory, every-20-turns condense) are closed.

**IMPLEMENTATION_QUALITY:** 93 (AI) / 93 (architecture)  
**LIVE_PROVIDER_CONFIDENCE:** LOW (credentials not exercised in this wave)
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Kai casual chat | Kai capsule only | 1 DeepSeek stream | 0 | 0 | 0 | 0–5 **relevant only** | event facts; no N-turn LLM | 500 text_tokens | `kaios_chat_stream` | AbortSignal | casual envelope | SSE error; **no** legacy | only if `KAIOS_RUNTIME=false` |
| Kai motivation | Kai | 1 DeepSeek stream | 0 | 0 | 0 | 0–5 relevant | same | 500 | `kaios_chat_stream` | yes | casual | error | soak only |
| Alex training question | Alex | 1 DeepSeek stream | 0 | optional searchExercises (backend) | none unless tool router | 0–5 relevant | same | 500 | `kaios_chat_stream` | yes | casual / form | error | soak |
| Alex programming | Alex | 1 DeepSeek **complete** (structured intent) | 0 | validateExerciseIds if tools used | none from prose | 0–5 relevant | same | 500 | `kaios_chat_structured` | yes | envelope + Zod | text-only envelope, not success invent | soak |
| Maya nutrition question | Maya | 1 DeepSeek stream | 0 | getNutritionState | pending confirm only via tools | 0–5 relevant | same | 500 | `kaios_chat_stream` | yes | casual | error | soak |
| Maya meal photo | Maya (synthesis) | **1 Gemini vision envelope + 1 DeepSeek synthesis** | 1 Gemini | none | pending analytics confirm if macros usable and not ambiguous | photo history optional | none auto | 1 maya_photo | `vision`, `synthesis` | signal on pipeline | combined envelope + `technicalAnalysisSchema` | refund photo credit; low quality stops before DeepSeek | soak uses same pipeline |
...
| Leo physique analysis | Leo synthesis | **1 Gemini + 1 DeepSeek** | 1 Gemini | physique history | score message persist | previous scores | none | 1 leo_photo | `vision`, `synthesis` | yes | schema + quality fail-closed | refund; Gemini is not final score | soak |
| Leo repeat-image | Leo | **0 Gemini / 0 DeepSeek** when valid cached observation (TTL 30d, user+type+fingerprint) | 0 | history | **no new score row** | prior analysis reused; `drift: []` | n/a | **0** extra photo credit | none | n/a | prior payload | failed analyses never reused | soak |
...
| Flow | Before (HEAD legacy) | After (KAIOS default, final closure) | User quota | Platform ledger |
| --- | --- | --- | --- | --- |
| Kai casual | 1 stream + optional card LLM + optional analytics LLM | **1** stream | reserved 500 then settle | `kaios_chat_stream` |
| Kai memory | same | **1** stream (0 memories injected if none relevant) | same | stream |
...
| Maya photo | quality + vision + synthesis (**3**) | **1 Gemini + 1 DeepSeek** | 1 photo credit | `vision`, `synthesis` |
| Leo photo | same 3 | **1 Gemini + 1 DeepSeek** | 1 photo credit | `vision`, `synthesis` |
| Leo exact-repeat | 3 | **0 Gemini** where reuse valid (TTL 30 days) | 0 extra | none |
| Council | 1 one-shot JSON array (legacy) | **1** turn complete | team reserve | `council_turn` |
| Memory | 1 condense / 20 turns | **NONE** automatic; event-fact rows; hint-gated `analytics` only | n/a | `analytics` if hint matches |

Exceptions: hint-gated `applyCoachAnalyticsFromChat` (metered `analytics`) when the user message looks like a numeric log — **not** a periodic summary. `condenseMemory` remains opt-in and unused on the chat path. Schema repair is deterministic (`extract-json`), not an extra LLM.

KAI_CASUAL_PROVIDER_CALLS: **1**  
ALEX_TEXT_PROVIDER_CALLS: **1**  
MAYA_TEXT_PROVIDER_CALLS: **1**  
MAYA_PHOTO_PROVIDER_CALLS: **≤2** (1 Gemini + 1 DeepSeek on success; 1 Gemini then stop on insufficient quality)  
LEO_PHOTO_PROVIDER_CALLS: **≤2**  
LEO_REPEAT_IMAGE_GEMINI_CALLS: **0** where valid cached observation exists  
SEPARATE_VISION_QUALITY_CALL: **NONE**  
COUNCIL_PROVIDER_CALLS: **1** per turn  
SECONDARY_UNMETERED_CALLS: **0**  
UNKNOWN_PROVIDER_CALLS: **0**  
...
| Case | Behavior |
| --- | --- |
| Relevant | score ≥ `MEMORY_RELEVANCE_THRESHOLD` (2) via coach/intent keywords + user-message overlap; cap **5**, no minimum |
| Irrelevant | **excluded** (not used to fill remaining slots) |
| Casual unrelated | overlap required; **0** memories is correct |
| Contradictory | canonical profile/analytics outrank memory (prompt: DATA only) |
| Malicious stored | `isPoisonMemory` drops jailbreak/privilege text before ranking |
| Stale | weak + older than 180 days excluded |
...
IRRELEVANT_MEMORY_INJECTION: **PASS**
...
Gemini JSON is untrusted observation. Combined envelope `{ quality, observations }` is interpreted fail-closed: missing/NaN/OOR quality or missing observations → `INVALID_PROVIDER_OUTPUT` (no default passing score). Finite low score → `INSUFFICIENT_QUALITY` (localized copy; **no DeepSeek**). Image-text injection guarded in vision prompts. Macros from vision remain `model_estimate`. Scores are not Gemini UX authority (Leo eval/synthesis). Hostile image text cannot authorize tools.

VISION_PROMPT_INJECTION: **PASS** (prompt + unit schema; live hostile-image MANUAL_CANARY)  
LEO_QUALITY_GATE: **PASS**
...
| Vision | 1 photo unit reserved **after** reuse miss; refund on fail; reuse skips quota |
...
**CHANGE:** KAIOS hard-stops card LLM; chat path hint-gates analytics extractor (`operation: "analytics"`). Photo path is one metered Gemini `vision` + one metered DeepSeek `synthesis`. Automatic 20-turn memory LLM removed.  
**STATUS:** FIXED  
**FILES_CHANGED:** `lib/ai/structured-chat.ts`, `lib/ai/usage-ledger.ts`, `lib/ai/model-router.ts`, `lib/services/analysis.service.ts`, `lib/services/chat.service.ts`, `lib/services/memory.service.ts`  
**RESIDUAL_RISK:** Hint-gated analytics still an extra LLM when the user logs numbers; live provider spend unmeasured.
...
**CLASSIFICATION:** CURRENT_KAIOS  
**CHANGE:** Combined Gemini envelope; `interpretVisionEnvelope` fail-closed. Zod score required finite 1–10. No default 7.  
**TESTS:** `tests/unit/image-quality-gate.test.ts`, `tests/kaios/vision-envelope.test.ts`, `tests/kaios/photo-pipeline.test.ts`  
**STATUS:** FIXED  
...
Centralized in `lib/ai/models.ts`; env adapters in `lib/ai/env.ts` fail loud on unknown IDs.

| Key | Value |
| --- | --- |
| PRIMARY_TEXT_PROVIDER_CONFIG | `DEEPSEEK_MODEL` (optional; default if unset) |
| PRIMARY_TEXT_MODEL | **deepseek-chat** |
| VISION_PROVIDER_CONFIG | `GEMINI_MODEL` (optional; default if unset) |
| VISION_MODEL | **gemini-flash-lite-latest** |
| DEFAULT | `deepseek-chat` / `gemini-flash-lite-latest` |
| PRODUCTION_REQUIRED_VALUE | same, or another **allowlisted** ID |
| TEST_VALUE | same as DEFAULT (no live calls in default Vitest) |
| Allowlist DeepSeek | `deepseek-chat`, `deepseek-reasoner` |
| Allowlist Gemini | `gemini-flash-lite-latest`, `gemini-flash-latest`, `gemini-2.0-flash`, `gemini-2.0-flash-lite`, `gemini-2.5-flash`, `gemini-2.5-flash-lite` |
| Fallback | **none** to a different vendor; invalid ID → `AiEnvError` |
...
IMPLEMENTATION_QUALITY: **93** (AI implementation; not live confidence)  
...
PRIMARY_TEXT_PROVIDER_CONFIG: **DEEPSEEK_MODEL**  
PRIMARY_TEXT_MODEL: **deepseek-chat**  
VISION_PROVIDER_CONFIG: **GEMINI_MODEL**  
VISION_MODEL: **gemini-flash-lite-latest**  
MODEL_CONFIG_CONTRACT: **PASS**  
KAI_CASUAL_PROVIDER_CALLS: **1**  
ALEX_TEXT_PROVIDER_CALLS: **1**  
...
AI_SCORE: **93/100**  
KAIOS_ARCHITECTURE_SCORE: **93/100**  
AI_SAFETY_SCORE: **93/100**  
AI_RELIABILITY_SCORE: **90/100**  
AI_COST_EFFICIENCY_SCORE: **92/100**  
COST_EFFICIENCY_PRODUCT_SCORE: **92/100**  
LIVE_DEEPSEEK: **NOT_AVAILABLE**  
LIVE_GEMINI: **NOT_AVAILABLE**  
LIVE_PROVIDER_CONFIDENCE: **LOW**  
TYPECHECK: **PASS**  
LINT: **PASS**  
TESTS: **PASS** (862 passed, 13 skipped; live KAIOS excluded)  
BUILD: **PASS**  
NPM_AUDIT_HIGH: **PASS** (0 vulnerabilities)  
BUNDLE_BUDGET: **PASS**  
EXTERNAL_ACTION_REQUIRED: Manual canary only

---

Wave 8 was **not** started. Production was **not** deployed. Legacy rollback (`KAIOS_RUNTIME=false`) was **not** removed. There is **no** automatic KAIOS → legacy fallback.
