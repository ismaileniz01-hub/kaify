-- Production gem_ledger lacks metadata column; perform_daily_check_in must
-- mirror earn_gems schema-drift handling (transaction_type vs type, optional metadata).
  ...
  if v_streak.last_check_in_date = v_today then
    ...
    if not v_already then
      ...
      if v_has_meta then
        ...
        execute v_sql
        using v_user_id, 10, 'daily_check_in', 'Daily check-in +10', v_daily_key;
      end if;
