-- Referral reward: gift Thunder aura skin to both parties on successful referral.
-- Invitee always receives Thunder; referrer receives Thunder only when within
-- the existing 24h velocity cap (same rule as gem reward).
...
declare
  ...
  v_referrer_cap    constant integer := 25;
  v_award_referrer  boolean := true;
  v_skin_id         constant text := 'thunder';
begin
  ...
  -- Invitee always gets Thunder skin (idempotent via PK).
  insert into public.user_market_inventory (user_id, item_id)
  values (p_referred_id, v_skin_id)
