/**
 * Dr. Maya resimlerindeki MAVİ arka planı temizler (v2 - mavi tespiti)
 * Mavi = B değeri R ve G'den belirgin şekilde yüksek
 * Flood fill ile kenarlardan başlayarak mavi arka planı bulur.
 */
...
async function processFile(inputPath, outputPath) {
  ...
  // 2. AŞAMA: Mavi arka plan tespiti
  // Mavi = B > R ve B > G (mavi ton baskın)
  // Ayrıca renkler arası fark da önemli
  const visited = new Array(width * height).fill(false);
  const isBg = new Array(width * height).fill(false);
  const stack = [];

  function isBlueBg(r, g, b) {
    // Mavi ton baskın mı?
    const blueDominant = b > r && b > g;
    // Mavi arka plan genelde orta-düşük parlaklıkta
    const notTooBright = (r + g + b) < 700;
    // Çok koyu değil
    const notTooDark = (r + g + b) > 100;
    return blueDominant && notTooBright && notTooDark;
  }
