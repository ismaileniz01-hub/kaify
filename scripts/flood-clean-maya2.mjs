/**
 * dr maya 2.png flood fill ile temizler.
 * Kenar rengi: R=45 G=96 B=154
 * Daha yüksek threshold ile daha fazla mavi alanı temizler.
 */
...
async function processFile(inputPath, outputPath) {
  ...
  function isBgColor(r, g, b) {
    // Ya kenar rengine yakın olacak, ya da mavi ton baskın olacak
    const dr = Math.abs(r - avgR);
    const dg = Math.abs(g - avgG);
    const db = Math.abs(b - avgB);
    const nearEdge = dr < threshold && dg < threshold && db < threshold;
    const isBlue = b > r && b > g;
    return nearEdge || isBlue;
  }
...
console.log("\ndr maya 2.png (mavi flood fill v2)");
