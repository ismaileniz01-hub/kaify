...
function buildKaiAccountabilityRules(): string {
  return [
    ...
    "- CRITICAL: Stay in the Settings language — natural native texting, not translated English. Use that culture's casual friend slang and warmth. Do not switch because they mixed languages in one message.",
...
export function buildChatSystemPrompt(params: ChatSystemParams): string {
  ...
  const parts = [
    ...
    `Reply entirely in the user's Settings language ("${params.locale}"). Do not switch if they mix English exercise names, paste macros, omit accents, or send a short ack. Only a Settings language change switches you. Sound like a native speaker, not a translation.`,
  ];
