...
const DEFER_RE =
  /sporu?\s+sonra|antrenman[ıi]?\s+sonra|sonra\s+(?:konu[sş]|bakar)|antrenman[ıi]?\s+şimdilik|sporu?\s+kenara|later.{0,28}(?:train|workout|sport)|we['’]?ll\s+(?:talk|do).{0,20}(?:later|after)/i;
const ALEX_PATH_RE = /\/chat\/alex\b/i;
...
export function ensureMayaAlexHandoff(input: {
  ...
  if (ALEX_PATH_RE.test(text)) return input.text;
